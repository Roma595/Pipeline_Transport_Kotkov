...
StationSearch::StationSearch(const Data& data) {
	this->data = &data;
}

std::vector<Data::ID> StationSearch::searchByName(const std::string& name) const {
...
std::vector<Data::ID> StationSearch::searchByWorkshopsPercent(float low_percent, float high_percent) const {
	std::vector<Data::ID> result;
...
std::vector<Data::ID> StationSearch::searchByID(const std::unordered_set<Data::ID>& id) const {
	std::vector<Data::ID> result;
