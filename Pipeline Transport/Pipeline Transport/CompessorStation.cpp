...
CompressorStation add_CompressorStation() {
	...
	CS.number_of_use_workshop = validity_enter(0, CS.number_of_workshop);
...
CompressorStation read_CS_from_file() {
	CompressorStation CS;
	std::ifstream file;
	file.open("compressor_stations.txt");
	if (file.is_open()) {
		file >> CS.name >> CS.number_of_workshop >> CS.number_of_use_workshop >> CS.effectiveness;
	}
	file.close();
